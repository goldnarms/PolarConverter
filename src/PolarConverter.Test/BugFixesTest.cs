...
namespace PolarConverter.Test
{
    ...
    public class BugFixesTest
    {
        ...
        {
            var hrmData = FilHandler.LesFraFil(string.Format(RotSti + "{0}", @"EmptyFiles\11101501.hrm"));
        ...
        public void Bugs01()
        {
            var hrmData = FilHandler.LesFraFil(string.Format(RotSti + "{0}", @"EmptyFiles\13021901.hrm"));
            var modus = hrmData.Contains("SMode") ? "SMode" : "Mode";
            var modusVerdi = StringHelper.HentVerdi("Mode=", 9, hrmData);
            var polarData = new PolarData
            {
